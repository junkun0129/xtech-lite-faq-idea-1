...
const QuestionDetailSidebar: React.FC<QuestionDetailSidebarProps> = ({ question, onClose }) => {
  return (
    <>
      <div 
        className={`fixed top-0 right-0 h-full w-[480px] max-w-full bg-[#fcfcff] shadow-2xl transform transition-transform duration-300 ease-in-out z-30 ${question ? 'translate-x-0' : 'translate-x-full'}`}
        onMouseDown={(e) => e.stopPropagation()} // Prevent map pan
      >
        {question && (
          <div className="h-full flex flex-col">
            <div className="flex justify-between items-center p-4 border-b border-gray-200 flex-shrink-0">
              <h2 className="text-lg font-bold text-gray-800">質問の詳細</h2>
              <button 
                onClick={onClose} 
