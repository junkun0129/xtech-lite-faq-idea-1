    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {question.tags.map(tag => (
          <span key={tag} className="bg-gray-200 text-gray-600 text-xs font-medium px-2.5 py-0.5 rounded-full">{tag}</span>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {bestAnswer && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-teal-800">{bestAnswer.author.name}</span>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {bestAnswer.author.tags.map(tag => (
                       <span key={tag} className="bg-teal-100 text-teal-700 text-xs font-medium px-2 py-0.5 rounded-full">{tag}</span>
                    ))}
                  </div>
           ...
           {question.answers.filter(a => !a.isBestAnswer).map(answer => (
            <div key={answer.id} className="bg-gray-100 border border-gray-200 rounded-lg p-4">
              <div className="flex items-start gap-3">
